...
pub struct TicketCollectionIter {
    inner: std::collections::btree_map::IntoIter<TicketId, Ticket>,
}

impl Iterator for TicketCollectionIter {
    type Item = Ticket;

    fn next(&mut self) -> Option<Self::Item> {
        self.inner.next().map(|(_, ticket)| ticket)
    }
}

impl IntoIterator for TicketStore {
    type Item = Ticket;
    type IntoIter = TicketCollectionIter;

    fn into_iter(self) -> Self::IntoIter {
        TicketCollectionIter {
            inner: self.tickets.into_iter(),
        }
    }
}
...
mod tests {
    ...
    fn works() {
        ...
        let ids: Vec<TicketId> = (<TicketStore as Clone>::clone(&store)).into_iter().map(|t| t.id).collect();
        let sorted_ids = {
            let mut v = ids.clone();
            v.sort();
            v
